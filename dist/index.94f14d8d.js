document.addEventListener('DOMContentLoaded', ()=>{
    ...
    todoList.addEventListener("click", (e)=>{
        if (e.target.classList.contains("element-delete")) e.target.closest(".element").remove();
    });
    todoSearch.addEventListener("input", ()=>{
        ...
        for (const el of elems){
            ...
            if (text.includes(val)) el.style.setProperty("display", "");
            else el.style.setProperty("display", "none");
        }
    });
});

//# sourceMappingURL=index.94f14d8d.js.map
